...
SPEC_GUIDED_TEST_DIR = 'spec-guided_tests'        # New directory for spec-guided tests
COVERAGE_REPORT_DIR = 'coverage_reports/spec-guided'  # Where to save spec-guided HTML reports

# Ensure directories exist
os.makedirs(SPEC_GUIDED_TEST_DIR, exist_ok=True)
os.makedirs(COVERAGE_REPORT_DIR, exist_ok=True)

# Function to run pytest with coverage on spec-guided tests and parse output
def run_spec_guided_coverage(problem_id, llm, sample_num):
    ...
    test_file = os.path.join(SPEC_GUIDED_TEST_DIR, f"test_{problem_id}_{llm}_sample{sample_num}.py")
    
    if not os.path.exists(code_dir) or not os.path.exists(test_file):
        print(f"Skipping {problem_id} ({llm} sample{sample_num}): Missing code or spec-guided test file.")
        return None  # Skip if files missing
    
    # Command to run pytest with coverage (using spec-guided test file only)
    cmd = [
        'pytest',
        '--cov=' + module_name,
        '--cov-branch',
...
results = []
for sel in SELECTED:
    result = run_spec_guided_coverage(sel['problem_id'], sel['llm'], sel['sample_num'])
    if result:
        results.append(result)

# Write to CSV
if results:
    with open('spec_guided_coverage.csv', 'w', newline='') as f:
    ...
    print("Spec-guided coverage table written to spec_guided_coverage.csv")
else:
    print("No results found for spec-guided tests.")